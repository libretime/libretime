...
def touch_timestamp():
    ...
    try:
        timestamp = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(int(os.path.getmtime(timestamp_file))))
    except OSError as e:
...
def watch(dir_id, directory):
    ...
    logging.info("Start scanning directory "+directory +
                 " for new files since " + timestamp)
    ...
    logging.info("{0} files found in DB in {1}:{2}".format(
        len(watched_files_id), dir_id, directory))
    ...
    for curroot, dirs, files in os.walk(watch_dir):
        ...
        for curFile in files:
            ...
            database["mtime"] = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(int(os.path.getmtime(curFilePath))))

            cur = conn.cursor()
            try:
                query = "SELECT * FROM cc_files WHERE filepath = %s AND directory = %s"
                cur.execute(
                    query, (database["filepath"], database["directory"]))
            ...
            if counter == 0:
                ...
                if airtime_md.analyse_file(curFilePath, database):
                    ...
                    logging.warning(
                        "Problematic file: {}".format(database["filepath"]))
            elif counter >= 1:
                ...
                new_mtime = datetime.datetime.strptime(
                    database['mtime'], "%Y-%m-%d %H:%M:%S")
                if fdate < new_mtime:
                    logging.info(
                        '--> Updating: {0}'.format(database["filepath"]))
                    database["utime"] = datetime.datetime.now()
                    try:
                        if airtime_md.analyse_file(curFilePath, database):
                            try:
                                ...
                                logging.error("Could not save data for {0}".format(
                                    database["filepath"]))
                                logging.error(e)
                                logging.error(traceback.format_exc())
                    except Exception as e:
                        logging.error("Could not analyse {0}".format(
                            database["filepath"]))
                        logging.error(e)
                        logging.error(traceback.format_exc())
                else:
                    logging.debug('No update required for {0}'.format(
                        database["filepath"]))
...
def connect_to_messaging_server():
    ...
    credentials = pika.credentials.PlainCredentials(
        config["rm_user"], config["rm_pass"])
    connection = pika.BlockingConnection(
        pika.ConnectionParameters(
            host=config["rm_host"],
            virtual_host=config["rm_vhost"],
...
def msg_received_callback(channel, method, properties, body):
    ...
    if "rescan_watch" in msg_dict["cmd"]:
        ...
        logging.info("Got message: "+msg_dict["cmd"]+" ID: "+str(
            msg_dict["id"])+" directory: "+msg_dict["directory"])
        try:
            watch(str(msg_dict["id"]), msg_dict["directory"])
        except Exception as e:
            logging.error("Unexpected error when calling watch() on {0}".format(
                msg_dict["directory"]))
            logging.error(e)
    else:
