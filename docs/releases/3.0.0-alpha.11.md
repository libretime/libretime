...
The general configuration schema has been changed:

- the `general.web_server_user` entry was removed
- the `general.cache_ahead_hours` entry now defaults to 1

The storage (`current_backend`) configuration schema has been changed:

- the `current_backend.storage_backend` entry now defaults to the only valid value `file`.

Feel free to remove the entire `current_backend` section from your configuration file.
...
The rabbitmq configuration schema has been changed:
