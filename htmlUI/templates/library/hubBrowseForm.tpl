...
{assign var="_style_textarea" value=" class='area_browse'"}
                ...
                <tr>
                <td style="border:0">
                    {foreach from=$hubBrowseForm.col1 item=dynform}
                        {include file="library/dynForm_browse.tpl"}
                    {/foreach}	
                </td><td style="border:0">
                    {foreach from=$hubBrowseForm.col2 item=dynform}
                        {include file="library/dynForm_browse.tpl"}
                    {/foreach}
                </td><td style="border:0">
                    {foreach from=$hubBrowseForm.col3 item=dynform}
                        {include file="library/dynForm_browse.tpl"}
                    {/foreach}
                </td>
                </tr>
                </table>
                    {foreach from=$hubBrowseForm.global item=dynform}
